...
void test_reuse_after_free()
{
    ...
    byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
        {{1, 2, 3}, {11, 12, 13, 14, 15, 16, 17, 18}, {20}, {21, 22, 23, 24, 25, 26, 27}, {31, 32, 33, 34, 35, 36, 37, 38, 39}, {91, 92}, {static_cast<byte>(LAST)}};
...
{
    if (!fork())
    { // test as son, to get a clear heap
        func();
        exit(0);
    }
    else
    { // father waits for son before continuing to next test
        int exit_status = 0;
        wait(&exit_status);
        if (exit_status)
