//
// Created by Ariel Kazula on 25/06/2021.
//
...
#include "malloc_4.h"
#include "os_hw4_part3_tests/colors.h"
...
typedef struct MallocMetadata4 {
	bool is_free;
	bool on_heap;
	size_t size;
	MallocMetadata4 *next;
	MallocMetadata4 *prev;

	MallocMetadata4 *next_free_on_list;
	MallocMetadata4 *prev_free_on_list;
	long offset;
} Metadata4;
...
#define DO_MALLOC(x) do{ \
		...
		if((unsigned long)(t) % 8 != 0){                \
           std::cerr << "Address is not multiple of 8. at line: "<< __LINE__ << ". command: "<< std::endl << #x << std::endl; \
           exit(1) ;\
        }                \
		checkSizeIsMultipleOf8<Metadata4>((Metadata4 *)((char *)(t)-size_of_metadata)); \
}while(0)
...
std::string allocNoFree(void *array[MAX_ALLOC]) {
	std::string expected = "|U:104|U:100000|U:16|";
	if (MAX_ALLOC < 4) {
	...
	printMemory4<Metadata4>(memory_start_addr, true);
	return expected;
}
...
Metadata4 * getMeta(void *address){
	return (Metadata4 *) ((char *) address - size_of_metadata);
}

int getSizeOfLeftOver(void *address, size_t size) {
	int offset = _getOffset((char *) address + size);
	size_t current_size = ((Metadata4 *) ((char *) address - size_of_metadata))->size;
	return ((int) current_size) - size - size_of_metadata - offset;
}
...
std::string testSplitAndMerge(void *array[MAX_ALLOC]) {
	...
	sfree(array[6]);
	checkStats(0, 0, __LINE__);
	DO_MALLOC(array[4] = srealloc(array[4], default_block_size + default_block_size / 3));

	checkStats(0, 0, __LINE__);
	DO_MALLOC(array[5] = srealloc(array[5], default_block_size + default_block_size / 3));
	checkStats(0, 0, __LINE__);
	sfree(array[10]);
	checkStats(0, 0, __LINE__);
...
std::string testCalloc(void *array[MAX_ALLOC]) {
	std::string expected = "|U:104|U:24|U:104|";
	if (MAX_ALLOC < 4) {
		std::cout << "Test Wont work with MAX_ALLOC < 4";
		return expected;
	}
...
std::string testFreeAllAndMerge(void *array[MAX_ALLOC]) {
	...
	allsize += _getOffsetFromMetadata(array[0]);
	checkStats(0, 0, __LINE__);
...
std::string testInit(void *array[MAX_ALLOC]) {
	...
	if (sizeof(Metadata4) != _size_meta_data()) {
...
std::string testReallocDecOverwrite(void *array[MAX_ALLOC]) {
	...
	printMemory<Metadata4>(memory_start_addr, true);
	return expected;
}

std::string testReallocWilderness(void *array[MAX_ALLOC]) {
...
TestFunc functions[NUM_FUNC] = {testInit, allocNoFree, allocandFree, testFreeAllAndMerge, allocandFreeMerge, testRealloc, testRealloc2, testWild,
								testSplitAndMerge, testCalloc, testBadArgs, testReallocMMap, testReallocDec,testReallocDecOverwrite, testReallocWilderness};
...
void checkStats(size_t bytes_mmap, int blocks_mmap, int line_number) {
	updateStats4<Metadata4>(memory_start_addr, current_stats, bytes_mmap, blocks_mmap);
...
void printStartRunningTests() {
	std::cout << "RUNNING TESTS: (MALLOC PART 4)" << std::endl;
